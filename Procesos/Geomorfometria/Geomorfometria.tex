...
\begin{keypoints}
¿Que características tiene un MDE y cómo se prepara para su análisis? $\bullet$ ¿Cómo podemos definir matemáticamente la superficie representada por un MDE a nivel local? $\bullet$ ¿Cómo usar esa definición matemática para derivar valores geométricos? $\bullet$ ¿Qué representan las direcciones de flujo y qué métodos hay para calcularlas? $\bullet$ ¿Cuáles son los principales parámetros relacionados con la estructura hidrológica de un MDE? $\bullet$ ¿Qué es una cuenca visual y cómo se calcula?
\end{keypoints}
...
\section{Creación y preparación del MDE}\label{Preparacion_MDE}
...
\item Splines. Los splines representan una de las mejores alternativas para la creación de MDE. Frente a las restantes metodologías, autores como \cite{Mitasova1993MathGeo} recomiendan el uso de splines para la obtención de MDE idóneos para el análisis.
\item Ajuste de funciones. Como métodos globales, ajustar una función de tipo polinómico a los puntos de datos y asemejar el relieve a esta no es adecuado, ya que el relieve es altamente más complejo . El uso de funciones de grado superior y mayor complejidad, por otra parte, hace que puedan aparecer valores extremos irreales que desvirtúen por completo la superficie interpolada (ver \ref{Ajuste_de_polinomios}). Aplicados como métodos locales, no obstante, veremos más adelante que este tipo de ajustes se utilizan no para la creación del MDE sino para el análisis de las propiedades de este, ajustando localmente funciones con los valores de un entorno definido de celdas.\index{Ajuste de polinomios}
\end{itemize}
...
\section{Modelos matemáticos locales del MDE}\label{Modelos_MDE}
...
\begin{eqnarray}\label{Eq:Parametros_Evans}
...
\begin{equation}\label{Eq:Zevenbergen} 
...
\begin{eqnarray}\label{Eq:Parametros_Zevenbergen}
...
\subsection{Medidas geométricas}\label{Medidas_geometricas}
...
\begin{equation}\label{Eq:PendienteEvans}
...
\begin{equation}\label{Eq:Notacion_derivadas}
...
\begin{equation}\label{Eq:Orientacion}
...
\begin{equation}\label{Eq:Orientacion2}
\begin{split}
...
\begin{equation}\label{Eq:Insolacion}
...
\begin{figure}[!hbt]
\centering
\includegraphics[width=.5\mycolumnwidth]{Geomorfometria/Insolacion.png}
\caption{\small Mapas de insolación total a lo largo de un año (en kWh/m$^2$).}
\label{Fig:Insolacion}
\end{figure}
...
\subsubsection{Medidas basadas en derivadas de segundo grado}\label{Curvaturas}
...
\begin{equation}\label{Eq:Curvatura_curva}
...
\begin{eqnarray}\label{Eq:Notacion_derivadas2}
...
\begin{equation}\label{Eq:Curvatura_vertical}
...
\begin{equation}\label{Eq:Curvatura_horizontal}
...
% \begin{figure}[h]
% \centering
% \includegraphics[width=.9\mycolumnwidth]{Curvaturas.png}
% \caption{\small  Mapas Curvatura horizontal (izquierda) y vertical (derecha)} 
...
\subsection{Medidas estadísticas}\label{Medidas_Estadisticas}
...
\begin{equation}\label{Eq:chA06:RoughnessHobson}
...
\begin{equation}\label{Eq:RugosidadHobson1}
{\rm X}_i=\sin (s) \cdot \cos (a)
\end{equation}

\begin{equation}\label{Eq:RugosidadHobson2}
{\rm Y}_i=\sin (s) \cdot \sin (a)
\end{equation}

\begin{equation}\label{Eq:RugosidadHobson3}
{\rm Z}_i=\cos (s)
\end{equation}
...
\section{Análisis hidrológico}\label{Analisis_hidrologico}
...
\begin{equation}\label{Eq:Pendiente_D8}
...
\begin{figure}[h]   
\centering
\includegraphics[width=.3\mycolumnwidth]{Geomorfometria/Zonas_planas_dir_flujo.png}
...
\subsubsection{Depresiones}\label{Depresiones}
...
\subsection{Área acumulada y parámetros derivados}\label{Area_acumulada}
...
\begin{figure}[h]   
\centering
\includegraphics[width=.5\mycolumnwidth]{Geomorfometria/Pendiente_acumulada.png}
\caption{\small Mapa de pendiente media aguas arriba}
\label{Fig:Pendiente_acumulada} 
\end{figure}
...
\begin{figure}[h]   
\centering
\includegraphics[width=.5\mycolumnwidth]{Geomorfometria/Longitud_flujo.png}
\caption{\small Mapa de longitud de flujo desde aguas arriba}
\label{Fig:Longitud_flujo} 
\end{figure}
...
\caption{\small División de una cuenca en subcuencas con puntos de salida en confluencias con variación de órdenes de a)}
...
%\begin{figure}[h]    
%\centering
%\includegraphics[width=.6\textwidth]{Subcuencas_mismo_area.png}
%\caption{\small División de una cuenca en subcuencas del mismo área.}
%\label{Fig:Subcuencas_mismo_area} 
%\end{figure}
...
% \begin{figure}[h]    
% \centering
% \includegraphics[width=.6\textwidth]{Tiempo_salida_variable.png}
% \caption{\small a) Mapa de tiempos de salida suponiendo velocidad variable. b) Histograma de frecuencias asociado.}
% \label{Fig:Tiempo_salida_variable} 
% \end{figure}
...
% \begin{figure}[h]      
% \centering
% \includegraphics[width=0.6\textwidth]{Indice_potencia_cauce.png}
% \caption{\small Mapa de potencia de cauce.}
% \label{Fig:Indice_potencia_cauce}
% \end{figure}
...
%Una representación de este índice la encontramos en la figura \ref{Fig:Factor_LS}

% \begin{figure}[h]      
% \centering
% \includegraphics[width=0.6\textwidth]{Factor_LS.png}
% \caption{\small Mapa de factor LS.}
% \label{Fig:Factor_LS}
% \end{figure}
...
\begin{figure}[!hbt]
\centering
\includegraphics[width=.5\mycolumnwidth]{Geomorfometria/Cuenca_visual.pdf}
\caption{\small Cuenca visual asociada a un punto dado (en rojo). Se ha supuesto una altura del objeto de 20 metros.} \label{Fig:Cuenca_visual}
\end{figure}
...
\begin{equation}\label{Eq:chA06:RelativeSize}
...
%\begin{figure}[!hbt]
%\centering
%\includegraphics[width=.6\mycolumnwidth]{Geomorfometria/Visibilidad_tamano.pdf}
%\caption{\small Tamaño relativo de un punto (en rojo) de 20 metros de alto, desde los puntos desde los cuales es visible.} \label{Fig:Visibilidad_tamano}
%\end{figure}
...
\caption{\small Mapa de exposición visual de un cauce, reflejando el número de celdas pertenecientes a este que son visibles desde cada una de las restantes, ponderadas por su importancia jerárquica en la red de drenaje.} \label{Fig:Exposicion_visual}
\end{figure}
...
\section{Caracterización de formas del terreno}\label{Caracterizacion_terreno}
...
% \caption{\small Caracterización de formas del relieve a partir de clasificación no supervisada}\label{Fig:Clasificacion_formas_relieve} 
% \end{figure}
