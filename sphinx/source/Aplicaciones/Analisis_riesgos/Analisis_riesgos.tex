...
Para el lector interesado, un software con carácter didáctico destinado a mostrar los fundamentos básicos de los Modelos de Circulación General es *EdGCM*, que puede descargarse de forma gratuita en la dirección Web \cite{EdGCM}
...
.. figure:: Variacion_area_TOPMODEL.png
...
.. _Fig:Variacion_area_TOPMODEL:


Se han propuesto modificaciones del índice de humedad topográfica encaminadas a modelizar de forma más realista el proceso, incorporando otras variables que también influyen en el mismo, principalmente otros parámetros morfométricos. En esta línea encontramos, por ejemplo, el *índice topográfico de humedad relativa* propuesto por \cite{Parker1982PG} o el índice elaborado por \cite{Fall2000RB}, en el que incorporan también la posición orográfica de la celda. El uso de estas, no obstante, es reducido.
...
.. figure:: Esquema_MDT_modelos_hidro.png
...
.. _Fig:Esquema_MDT_modelos_hidro:
...
.. figure:: Ponderacion_Thiessen.png
...
.. figure:: Celda_modelo_distribuido.png

	Caracterización de cada celda como una unidad independiente desde el punto de vista de sus características físicas. La base para el desarrollo de modelos distribuidos reside fuertemente en esta consideración (Adaptado de \cite{Wigmosta1999WRR)}


.. _Fig:Celda_modelo_distribuido:
...
Empleando el concepto de volumen de control \cite{Chow1988McGraw}, tres son las formas principales de tratar el conjunto de celdas de una cuenca para lograr los valores finales del modelo correspondientes a esta\cite{Asante2000PhD}:
	...
	* Metodologías basadas en transmisión celda a celda. Fundamentadas en el análisis del balance hidrológico en cada celda y las de su entorno inmediato. Se analiza cada celda según el esquema mostrado en la figura \ref{Fig:Balance_hidrico_celda}.
...
%La figura \ref{Fig:Esquema_delimitacion_zonas_inundacion} muestra una visión esquemática de todo el proceso tratado en esta sección, en el que puede verse cómo desde los datos de partida se alcanza un resultado cartográfico final en el cual puede ya interpretarse los riesgos de avenidas que un evento dado comporta. Se han señalado aquellas partes del proceso donde la utilización de un SIG aporta mayores ventajas con respecto a la metodología clásica.
%
%.. figure:: Esquema_delimitacion_zonas_inundacion.pdf
%
	 Esquema funcional del proceso de delimitación de zonas de inundación, señalando aquellos pasos en los que el empleo de SIG resulta más relevante.
%

.. _Fig:Esquema_delimitacion_zonas_inundacion:
%
...
A pesar de todo ello, las capacidades SIG de estos programas no son tan versátiles como las de un SIG genérico, y el uso de los mismos queda muy restringido al propio estudio de procesos hidrológicos e hidráulicos. %La figura \ref{Fig:Mike} muestra el aspecto de una de estas aplicaciones.

%.. figure:: Mike.png
%
	 Interfaz de usuario de MIKE, una aplicación para análisis hidrológico e hidráulico con elementos SIG.
%
...
El uso exclusivo de la pendiente es una primera opción que, a rasgos generales, puede indicar el condicionamiento que este parámetro ---no el único implicado, aunque quizás el más importante--- plantea de cara a la estabilidad del terreno representado por las distintas celdas. El valor de :math:`35\degree`, denominado habitualmente <<ángulo de reposo>>, es empleado con frecuencia para establecer una división sencilla entre las celdas de pendiente mayor que dicho valor ---con riesgo de deslizamiento--- o menores ---sin riego---. Una sencilla reclasificación de una capa de pendientes nos da así una primera aproximación a una capa de riesgo de desplazamientos en masa.

Si introducimos otras variables además de la pendiente, podemos obtener resultados más refinados. \cite{Pack1999SINMAP} propone en su modelo SINMAP un índice de estabilidad basado en la teoría de *pendiente infinita* \cite{Hammond1992USDA} ---también conocida como de *talud infinito*---, la cual define un factor de seguridad como la relación entre las fuerzas gravitatorias ---desestabilizantes--- y las de cohesión ---estabilizantes---.

.. figure:: SINMAP.png

	Valores de índice de estabilidad en el modelo SINMAP en función de área y pendiente (adaptado de \cite{Pack1999SINMAP).}
...
\begin{table}[!hbt]
	\begin{center}
		\begin{tabular}{cp{8cm}} \toprule
			...
			IE:math:`>`1,5 & Zona estable \\ 
			1,54:math:`>`IE:math:`>`1,25 & Zona moderadamente estable \\ 
			1,25:math:`>`IE:math:`>`15 & Zona casi estable. A partir de este punto se pueden producir deslizamientos en masa. \\ 
			1:math:`>`IE:math:`>`0,5 & Zona inestable. Menos del 50\% probabilidad de que se produzca deslizamiento en función de los valores de parámetros considerados. \\ 
			0,5:math:`>`IE:math:`>`0 & Zona inestable. Menos del 50\% probabilidad de que se produzca deslizamiento \\ 		
			0:math:`>`IE & Zona incondicionalmente inestable\\ \bottomrule
		\end{tabular}
	\end{center}
	
	Clases de estabilidad según el modelo SINMAP \cite{Pack1999SINMAP}
...
.. figure:: Incendio_MonteCarlo.png
...
.. figure:: Esquema_riesgo_incendio.pdf

	Esquema funcional del análisis de riesgo de incendios.


.. _Fig:Esquema_riesgo_incendio:
