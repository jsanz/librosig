...
.. figure:: Electromagnetic_spectrum-es.pdf

	Espectro electromagnético y sus principales regiones de interés (Tomado de Wikipedia).


.. _figespectro_electromagnetico: 


Dentro de estas regiones, son de destacar las siguientes:


	* Rayos :math:`\gamma`  :math:`<`0.03 :math:`nm`.
	* Rayos X (0.03 :math:`nm` - 3 :math:`nm`).
	* Ultravioleta (3 :math:`nm` - 0.3 :math:`\mu`).
	* Visible (0.3 :math:`\mu` - 0.7 :math:`\mu`). Se corresponde con las radiaciones que pueden ser detectadas por el ojo humano o por aparatos tales como una cámara fotográfica común. Observando la figura :num:`#figespectroelectromagnetico` puede apreciarse que esta región representa una porción muy pequeña del total del espectro. Existen muchas otras regiones que no podemos *ver* pero que, empleando la tecnología adecuada, sí que pueden aprovecharse para crear imágenes dentro de un proceso de teledetección, siendo de gran utilidad para el estudio de numerosos procesos.
	...
	* Infrarrojo cercano (0.7 :math:`\mu` - 1.3 :math:`\mu`).
	* Infrarrojo medio  (1.3 :math:`\mu` - 8 :math:`\mu`).
	* Infrarrojo lejano o térmico (8 :math:`\mu` - 14 :math:`\mu`). Dentro de esta región se encuentran principalmente las radiaciones emitidas por los cuerpos debido a su temperatura\footnote{Esta emisión se calcula según la denominada *ley de Stefan--Boltzmann*. Puede encontrarse más al respecto en \cite{webSBoltzman}}.
	* Microondas (1 :math:`mm` - 25 :math:`cm`).
...
.. figure:: Orbita_landsat.png

	Esquema de barrido de un satélite con órbita heliosíncrona. Tomado de \cite{webLandsat}
...
A continuación se relacionan algunos de los sistemas de teledetección principales y las características de sus productos.
	...
	* IKONOS \cite{webIkonos}. Este satélite, lanzado en 1999, monta un sensor con resolución de 1 metro para imágenes pancromáticas y 4 metros para imágenes multibanda (4 bandas). Las imágenes cubren una área de 11 :math:`\times` 11 km y el satélite tiene una resolución temporal de entre 3 y 5 días.
	...
	* QuickBird. \cite{webQuickbird}. Ofrece imágenes en pancromático y multibanda (azul, verde, rojo e infrarrojo cercano). Las primeras tiene una resolución de 60 cm y las multibanda de 2,4 metros, aunque combinando las dos ofrece imágenes en color con 60 cm de resolución. 
	...
	* RADARSAT. Desarrollado por la Agencia Espacial Canadiense, monta un radar de apertura sintética (SAR), y su principal propósito es el control de las variaciones ambientales y de los recursos naturales. Más información en \cite{webRADARSAT}.
	* ERS--1 y ERS--2. Desarrollados por la Agencia Espacial Europea. Al igual que el anterior, ambos están pensados para la observación medioambiental, y montan tanto sensores activos como pasivos. Más información en \cite{webERS2}.
...
.. figure:: Digitalizacion_en_pantalla.png
...
.. _figdigitalizacion_en_pantalla: 
...
.. figure:: Correccion_digitalizacion.png
...
.. figure:: Escaner_sobremesa.png

	Escáner de sobremesa (tomado de Wikipedia)


.. _figescaner_sobremesa: 


.. figure:: Escaner_tambor.png
...
El parámetro base es la relación entre el tamaño de píxel (la longitud real que representa el ancho de un píxel sobre el terreno) y el tamaño de este píxel en la imagen (lo que mide esa longitud en el mapa). Las resoluciones habituales utilizadas para el escaneo de fotografías aéreas varían entre los 100 dpi (:math:`\approx 250 \mu m` cada punto sobre el mapa) y 2500 dpi ((:math:`\approx 10 \mu m` cada punto sobre el mapa) \cite{Welch1996Onward}. Por ejemplo para una resolución de 300 dpi, se tiene:
...
Las formas de conseguir que el observador perciba la profundidad de la escena a partir de las imágenes son variadas, y van desde el uso de sencillos instrumentos ópticos o la generación de anaglifos (imágenes que combinan la información del par estereoscópico y que se han de observar con gafas con filtros distintos para cada ojo), hasta otras técnicas más complejas y elaboradas. En la fotogrametría no digital, el empleo de restituidores analíticos %como el mostrado en la figura :num:`#figrestituidoranalitico` 
...
%.. figure:: Restituidor_analitico.png
%
	Restituidor analítico.
%

.. _figrestituidor_analitico: 
%

.. figure:: Estacion_fotogrametrica_digital.png

	Estación fotogramétrica digital.


.. _figestacion_fotogrametrica_digital: 
...
.. figure:: Imprecisiones_digitalizacion.pdf
...
.. _figimprecisiones_digitalizacion: 
...
La digitalización de entidades en caso de querer recoger la topología de las mismas debe obedecer una serie de reglas, a saber\cite{GrassDigitizing}:
...
El funcionamiento del sistema se basa en la triangulación de la posición mediante las señales procedentes de un cierto número de los satélites. Esta posición se calcula no únicamente en sus coordenadas *x* e *y*, sino también en *z*, es decir en elevación. El sistema GPS emplea como sistema geodésico de referencia el WGS84 \cite{WGS84}. La precisión en el cálculo de la elevación es menor que la correspondiente a las restantes coordenadas, aunque también es de utilidad y puede emplearse en aplicaciones que van desde levantamientos y replanteos a usos en tiempo real como el cálculo de elevación en vuelos \cite{Graas1991Navigation}.
...
Una forma habitual de resolver esto es acompañar cada fichero de imagen con un pequeño fichero de texto plano donde se contengan los datos geográficos correspondiente a la imagen. Este fichero se denomina \extr{World File}, y tiene una forma como la siguiente:
...
.. figure:: Compresion_con_perdidas.png
