...
* ``http:`` El protocolo a usar, que define la forma en que se van a comunicar cliente y servidor. Aunque este es el más habitual, existen muchos otros tales como ``ftp`` o ``mailto``. Puede encontrarse más acerca de estos protocolos en \cite{protocolosWeb}.
...
AJAX \footnote{Asynchronous JavaScript And XML} \cite{garrett2005ajax} es una técnica de desarrollo muy popular en este sentido, y de la que los SIG Web hacen uso habitualmente. La figura :num:`#figajax` muestra una comparación entre el esquema de una aplicación Web tradicional y una basada en AJAX. 
