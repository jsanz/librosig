...
Supongamos la dirección Web \texttt{http://wiki.osgeo.org/wiki/Libro\_SIG}, en la cual puedes encontrar información relacionada con este libro e incluso descargarlo. Si visitas esa página estás efectuando una petición a través de esa URL, la cual se compone de las siguientes partes:
	...
	* \texttt{http:} El protocolo a usar, que define la forma en que se van a comunicar cliente y servidor. Aunque este es el más habitual, existen muchos otros tales como \texttt{ftp} o \texttt{mailto}. Puede encontrarse más acerca de estos protocolos en \cite{protocolosWeb}.
	* \texttt{wiki.osgeo.org} Esta cadena identifica la máquina donde reside la página que buscamos. Es en realidad una versión más legible para el ojo humano de un código numérico que indica la dirección concreta. El navegador lo convierte en realidad en algo como 128.118.54.228.
	* \texttt{wiki/Libro\_SIG} La página que buscamos dentro de todas las que hay en esa máquina. Se expresa como una ruta a partir del directorio raíz del servidor, que no es necesariamente el directorio raíz de la maquina servidora.
...
Una primera y sencilla clasificación de los tipos de cartografía Web es la que divide esta en mapas *estáticos* y *dinámicos*\cite{Kraak2001Francis}. 
...
AJAX \footnote{Asynchronous JavaScript And XML} \cite{garrett2005ajax} es una técnica de desarrollo muy popular en este sentido, y de la que los SIG Web hacen uso habitualmente. La figura \ref{Fig:AJAX} muestra una comparación entre el esquema de una aplicación Web tradicional y una basada en AJAX. 

.. figure:: ajax.png

	Comparación entre el esquema de una aplicación Web tradicional y una basada en AJAX.(adaptado de \cite{garrett2005ajax).}
...
La figura \ref{Fig:Tiger} muestra el aspecto de una aplicación de *Web Mapping* previa a la introducción de tecnologías como AJAX, en particular la Web a través de la que se accedía a los datos del proyecto TIGER, creado por el U.S Census Bureau.
...
.. figure:: tipos_cartografia_web.pdf

	Evolución de los tipos de cartografía en la Web (según \cite{Kraak2001Francis)}


.. _Fig:Tipos_Cartografia_Web: 
...
.. figure:: Servidores_y_clientes.pdf

	Relación entre clientes y servidores.


.. _Fig:Servidores_y_clientes: 
...
.. figure:: Datos_y_procesos_remotos.pdf
