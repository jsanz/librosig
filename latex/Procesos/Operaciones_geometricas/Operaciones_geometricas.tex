\chapter{Operaciones geométricas con datos vectoriales}\label{Operaciones_geometricas}

\begin{keypoints}
¿Cómo calcular una región de influencia a partir de una capa vectorial?$\bullet$¿Cómo combinar capas de polígonos con distintos tipos de variables?$\bullet$¿Qué representan la unión, la intersección y la diferencia entre capas vectoriales de polígonos?$\bullet$¿Qué tipos de envolventes mínimas se usan habitualmente, y cual es su significado?$\bullet$¿Para qué sirve simplificar un conjunto de líneas y cómo se lleva a cabo?$\bullet$¿Qué significa \emph{disolver} una capa de polígonos?
\end{keypoints}
...
\section{Zonas de influencia}\label{Zona_influencia_vectorial}
...
\caption{\small Zona de influencia a partir de puntos. La influencia puede analizarse a partir de los puntos que la ejercen (a) o los que la reciben (b)}\label{Fig:Zona_influencia_circular}
\end{figure}
...
\caption{\small Zona de influencia a partir de elementos lineales (cauces).}\label{Fig:Zona_influencia_lineas}
\end{figure}
...
\caption{\small Zona de influencia a partir de polígonos. a) Hacia el exterior,b) hacia el interior.} \label{Fig:Zona_influencia_poligonos}
\end{figure}
...
\caption{\small Zonas de influencia concéntricas a partir de una entidad lineal} \label{Fig:Zonas_influencia_concentricas}
\end{figure}
...
\caption{\small Zonas de influencia variable establecidas en función de los ordenes de los cauces.} \label{Fig:Zona_influencia_variable}
\end{figure}
...
\caption{\small Zona de influencia única a partir de un conjunto de zonas de influencia individuales que se solapan.} \label{Fig:Zona_influencia_disolver}
\end{figure}
...
%\begin{figure}[h]
%\centering
%\includegraphics[width=.65\mycolumnwidth]{Operaciones_geometricas/Zona_influencia_convolucionado.pdf}
%\caption{\small Zona de influencia de un polígono muy convolucionado.}\label{Fig:Zona_influencia_convolucionado}
%\end{figure}

%Es necesario en tal caso aplicar una disolución para evitar este tipo de solape.\index{disolución}
...
Estas operaciones pueden entenderse como operaciones de conjuntos y visualizarse mediante sencillos diagramas, de forma similar a lo que veíamos en la sección \ref{Mecanismos_consulta} al presentar los diagramas de Venn (Figura \ref{Fig:Operaciones_solape}).\index{Venn!diagrama de}

\begin{figure}[!hbt]
\centering
\includegraphics[width=\textwidth]{Operaciones_geometricas/Operaciones_solape.pdf}
\caption{\small Tipos de operaciones de solape entre dos elementos individuales.} \label{Fig:Operaciones_solape}
\end{figure}
...
\caption{\small Recorte vectorial de un conjunto de capas de puntos, líneas y polígonos, con una capa de polígonos.} \label{Fig:Recorte_vectorial}
\end{figure}
...
\caption{\small Interseccion entre dos capas de polígonos, mostrando las geometrías y la tabla de atributos resultante.}\label{Fig:Interseccion_vectorial}
\end{figure}
...
\caption{\small Unión de dos capas de polígonos, mostrando las geometrías y la tabla de atributos resultante.}\label{Fig:Union_vectorial}
\end{figure}

\subsection{Polígonos espúreos}\label{Poligonos_espureos}
...
\caption{\small Aparición de polígonos espúreos de pequeño tamaño tras el solape entre capas con discrepancias.}\label{Fig:Poligonos_espureos}
\end{figure}

\section{Juntar capas}\label{Juntar_capas}
...
\caption{\small Ejemplo de las tablas y entidades resultantes tras juntar dos capas.} \label{Fig:Ejemplo_juntar_capas}
\end{figure}
...
\caption{\small Obtención de una capa de comunidades autónomas a partir de una de provincias mediante un proceso de disolución. En la capa original se han representado los polígonos en función de la comunidad a la que pertenecen. Aunque aparecen con el mismo color, son polígonos independientes.}\label{Fig:Disolver}
\end{figure}
...
\subsection{Envolvente convexa mínima (convex hull)}\label{Convex_hull}
...
\caption{\small Contorno convexo mínimo.} \label{Fig:Convex_hull}
\end{figure}
...
\caption{\small Serie de contornos convexos formando una teselacion en capas (\emph{onion peeling}).} \label{Fig:Onion_peeling}
\end{figure}
...
\caption{\small Círculo de radio mínimo que engloba a una serie de puntos.}\label{Fig:Circulo_minimo}
\end{figure}
...
\caption{\small Rectángulo mínimo que engloba a un conjunto de puntos.}\label{Fig:Rectangulo_minimo}
\end{figure}
...
\section{Generalización de líneas}\label{Generalizacion_lineas}
...
\begin{figure}[h]
\centering
\includegraphics[width=.9\mycolumnwidth]{Operaciones_geometricas/Simplificacion_importancia_ptos.pdf}
...
\begin{itemize}
	\item Rutinas de vecindad inmediata. Analizan cada punto y los inmediatamente anteriores y posteriores, estudiando los ángulos formados o las distancias.
	\item Rutinas de vecindad acotada. Estudian una serie de puntos vecinos, no necesariamente restringida a los inmediatos pero sí con algun umbral máximo de alejamiento o número de puntos.
	\item Rutinas de vecindad no acotada. Estudian los puntos vecinos y la región analizada depende de diversos factores, aunque no existe una dimensión máxima.
	\item Rutinas globales. Analizan la línea de forma global, no a nivel de los puntos que la componen.
	\item Rutinas que preservan la topología. Además de las propiedades geométricas ya sea a nivel global o local, estudian la conectividad y topología del conjunto de líneas para garantizar que la línea simplificada preserva dichas propiedades.
\end{itemize}
...
\begin{itemize}
	\item Media entre puntos. Se consideran un número $n$ de puntos vecinos, y en base a ellos se calcula la nueva posición. 
	\item Ajuste de funciones matemáticas. Ajustando funciones a los puntos que tengan un aspecto <<suave>>, tales como \emph{splines} o curvas Bézier.\index{Splines}\index{Curva!de Bezier}
	\item Tolerancias. Se establece una tolerancia y un umbral de precisión, y se ignoran los detalles a lo largo de la línea que salen de ese umbral.
\end{itemize}
