...
\begin{keypoints}
¿Qué entendemos por \emph{herramientas de escritorio}? $\bullet$ ¿Por qué se caracterizan y cuáles son las tendencias actuales en su desarrollo? $\bullet$ ¿Qué funciones desempeñan dentro del conjunto de herramientas SIG? $\bullet$ ¿Qué tipo de tareas podemos desempeñar con ellas?
\end{keypoints}
...
\subsection{Visualización}\label{Funcion_SIG_Visualizacion}
...
En el caso más habitual, la representación de una capa en un lienzo de un SIG es bidimensional, de la misma forma que se representa en un mapa impreso, lo cual se debe tanto a la mayor facilidad de implementación de este tipo de representaciones como a la mayor exigencia que otro tipo de representaciones presentan en lo referente al equipo (\extr{hardware}). 
...
Estándares para el almacenamiento de estilos como SLD(\extr{Styled Layer Descriptor}), que veremos en detalle en el apartado \ref{SLD}, tienen como objeto solventar este problema. \index{SLD}
...
Debe pensarse que el proceso presentado como ejemplo es muy sencillo y únicamente implica cuatro operaciones encadenadas de forma lineal. Un proceso de análisis habitual puede contener muchas más operaciones individuales, y estas disponerse de forma más compleja, con dependencia de distinta índole entre sus resultados. La imagen \ref{Fig:Proceso_complejo} muestra el aspecto de uno de tales procesos implementado en un software (ArcGIS, véase sección \ref{Soft_privativo}) con capacidad de de creación de modelos. El proceso representado en la figura \ref{Fig:Esquema_MonteCarlo} también es un ejemplo de otro tipo de análisis que puede adaptarse ventajosamente en este tipo de herramientas de modelización.\index{ArcGIS}
...
\subsection{Generación de cartografía}\label{GeneracionCartografia}
